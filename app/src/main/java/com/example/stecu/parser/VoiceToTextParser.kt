package com.example.stecu.data.parser
...
class AndroidVoiceToTextParser(
    ...
    fun startListening(languageCode: String = "id-ID") {
        ...
        if(!SpeechRecognizer.isRecognitionAvailable(app)) {
            _state.update { it.copy(error = "Pengenal suara tidak tersedia") }
            return
        }
    ...
    override fun onResults(results: Bundle?) {
        results
            // Ganti RecognizerIntent dengan SpeechRecognizer
    ...
    override fun onPartialResults(partialResults: Bundle?) {
        partialResults
            ...
            ?.getOrNull(0)
            ?.let { partialText ->
                _state.update { it.copy(partialSpokenText = partialText) }
            }
    }
